# Redline v11 speaker notes · 22 slides
...
The live demo has no slide. It runs verbally between the personas slide (19) and the build plan (20); the run script is in presenter-prep.md.
...
The slide plays it on its own: the frame opens as submitted, the editor button presses itself at about two seconds, then Redline works the frame for about five seconds, the shimmer and the "Applying changes" pill, before the artwork softly resolves to the corrected frame around nine seconds, the v2 chip lands, and the first card's checkbox ticks itself. Let the working phase breathe; it's the honest beat, the model actually doing the work. Then: this is your March submission, this exact sticky. The frame redrew itself, the accepted line landed in the caption, and the third card's box is still open because marketing hasn't decided on the Frame 4 alt. It doesn't guess.
...
## 15 · Communicates and receives information where you do. (THE EDITOR)
...
- Thread matching keys on the part number in the subject. No part number? Redline infers and asks for confirmation. It never files silently.
- @-actions are allowlisted to submission verbs: file, extract, draft, assign, status. Anything that exports or submits still requires in-app approval by a human, same rule as everywhere else in the product.
- Precision on mechanism: in email you CC or address Redline with an instruction; "@redline" is the Google Chat mechanism, where @ is real.
...
## 17 · Redline drafts. Your team decides.
...
## 18 · Built to survive your MSA review
...
**Mobile beat, one line:** "The night in LA would have been one text to Redline from the passenger seat." Frame it honestly: Redline is a web app that works from a phone: status, approvals, and urgent prompt-changes. Do not claim a native app, offline mode, or mobile editing beyond that.

Then the live run. Pre-run script is in presenter-prep.md. Demo asset names live here, not on slides:
- Asset 1: [Cal selects from Nikki's attachments]
- Asset 2: [Cal selects from Nikki's attachments]
Run the four beats verbally: build, annotate, read, reconcile. If the model is slow, say so plainly: "this step takes about a minute. That minute replaces an afternoon." When the deck is forwarded, the link keeps working; sanitized data only behind it.
